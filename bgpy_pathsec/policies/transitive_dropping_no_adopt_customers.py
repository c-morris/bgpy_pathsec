            ...
            for as_ in self.customers:
                if not (isinstance(as_, TransitiveDroppingAS)):
                    ...
                    if origin:
                        ...
                        best_ann = as_._select_best_ribs_in(
                            Prefixes.PREFIX.value
                        )
                        # Re-process ribs_in with new AS class
                        current_ann = as_._local_rib.get_ann(
                            Prefixes.PREFIX.value
                        )
                        if (
                            best_ann is not None
                            and not best_ann.prefix_path_attributes_eq(
                                current_ann
                            )
                        ):
...
class TransitiveDroppingNoAdoptCustomersAlwaysAS(
...
class TransitiveDroppingNoAdoptCustomers2AS(
    TransitiveDroppingNoAdoptCustomersAS
):
    """Drops transitive attributes with some probability"""
...
class TransitiveDroppingNoAdoptCustomers4AS(
    TransitiveDroppingNoAdoptCustomersAS
):
    """Drops transitive attributes with some probability"""
