...
class BGPsecAS(BGPAS):
    ...
    def _process_outgoing_ann(self, as_obj, ann, *args):
        ...
        super(BGPsecAS, self)._process_outgoing_ann(
            as_obj,
            ann.copy(
                overwrite_default_kwargs={"next_as": next_as}
            ),  # noqa E501
            *args
        )
    ...
    def _security_second_new_ann_better(
        self,
        current_ann,
        current_processed,
        ...
        new_rel_better: opt_bool = self._new_rel_better(
            current_ann,  # noqa F821
            current_processed,
    ...
    def _new_as_path_ties_better(
        ...
        if new_as_path_shorter is not None:
            ...
            if self.security_third:
                ...
                if bgpsec_better is not None:
                    return bgpsec_better
            return self._new_wins_ties(
                current_ann, current_processed, new_ann, new_processed
            )

    def _new_ann_better_bgpsec(
        self, current_ann, current_processed, new_ann, new_processed
    ):
        ...
        current_ann_valid = (
            current_bgpsec_path == current_path
            and current_ann.next_as == self.asn
        )  # noqa E501
        new_ann_valid = (
            new_bgpsec_path == new_path and new_ann.next_as == self.asn
        )  # noqa E501
