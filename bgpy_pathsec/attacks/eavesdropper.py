...
class Eavesdropper(ShortestPathExportAll):
    ...
    def post_propagation_hook(self, engine, propagation_round, *args, **kwargs):
        ...
        if propagation_round == 0:
            attack_anns = []
            attacker = engine.as_dict[attacker_asn]
            if self.scenario_config.global_eavesdropper:
            ...
            for asn in [attacker_asn] + vantage_points:
                ...
                if (
                    ...
                    continue
                for ann_info in current_as._ribs_in.get_ann_infos(
                    Prefixes.PREFIX.value
                ):
                    if ann_info.unprocessed_ann.next_as == current_as.asn:
                        # if this ann has a signature on it, we have to process it first
                        atk_ann_candidates.append(
                            attacker._copy_and_process(
                                current_as._copy_and_process(
                ...
                #         atk_ann_candidates.append(attacker._copy_and_process(potential_ann_to_add, Relationships.CUSTOMERS))
