...
class IntentionalLeak(MHLeak):
    ...
    def post_propagation_hook(self, engine, propagation_round, *args, **kwargs):
        ...
        attacker_ann = engine.as_dict[attacker_asn]._local_rib.get_ann(
        ...
        if propagation_round == 0:
            attack_anns = []
            for ann_info in attacker._ribs_in.get_ann_infos(
                Prefixes.PREFIX.value
            ):
                atk_ann = attacker._copy_and_process(
                    ann_info.unprocessed_ann, Relationships.CUSTOMERS
                )
                ...
                if prev_len != len(atk_ann.as_path):
                    atk_ann = attacker._copy_and_process(
                        atk_ann, Relationships.CUSTOMERS
                    )
    ...
    def leak_announcements_to_providers(
        ...
        for neighbor in attacker.providers:
            ...
            if current_best_ann is not None:
                ...
                neighbor.process_incoming_anns(
    ...
    def _truncate_ann_hash(self, ann):
        ...
        if (
            len(ann.as_path) < 2
            or (
                len(ann.bgpsec_path) > 1
                and ann.as_path[0] == ann.bgpsec_path[0]
                and ann.as_path[1] == ann.bgpsec_path[1]
            )
        ...
        while i < len(ann.bgpsec_path) and j + 1 < len(ann.as_path):
            while ann.bgpsec_path[i] != ann.as_path[j] and j + 1 < len(
                ann.as_path
            ):
                j += 1
            if (
                j > 0
                and i + 1 < len(ann.bgpsec_path)
                and (
                    j + 1 == len(ann.as_path)
                    or ann.bgpsec_path[i + 1] == ann.as_path[j + 1]
                )
            ):
