...
class OverheadBPOAllSubgraph(Subgraph):
    ...
    def _add_traceback_to_shared_data(
        ...
        bpo_count = engine.as_dict[list(scenario.victim_asns)[0]].bpo_count
        overhead_count = engine.as_dict[list(scenario.victim_asns)[0]].count
        # Set the number of adopting ASes that were converted to non-adopting
        if scenario.attacker_asns:
            ...
            transitive_dropping_conversions_count = 0
        shared[
            "transitive_dropping_conversions_all"
        ...
        for as_obj, outcome in outcomes.items():
            if as_obj.asn in uncountable_asns:
                continue
            ribs_in = list(
                as_obj._ribs_in.get_ann_infos(scenario.announcements[0].prefix)
            )
